...
def place_order(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            data.order_total = total       #grand_total
            data.tax = GST
            ...
            ramount = total * 100   #RAZORPAY only accepts paisa not rs. ex: 100rs*100= 10,000 paisa
            request.session["razorpay_amount"] = ramount
            razorpay_client = razorpay.Client(auth=(settings.RAZOR_KEY_ID, settings.RAZOR_KEY_SECRET))
                
            razorpay_order = razorpay_client.order.create(
                dict(amount=ramount, currency=currency, payment_capture="0")
            )         
            
            #CHECKING FOR payment_type IS RAZORPAY
            if payment_type == "Razorpay":
                order_number = razorpay_order['id']
                request.session['order_number'] = order_number
                data.order_number = razorpay_order['id']
                # data.STATUS = payment_type
                data.save()
                
            #CHECKING FOR payment_type IS PAYPAL    
            elif payment_type == "PayPal":
                order_number = current_date + "_" + str(data.id)
                request.session['order_number'] = order_number
                data.order_number = order_number
                data.save()
            
            #CHECKING FOR payment_type IS COD OR NOT
            else:
                order_number = current_date + "_" + str(data.id)
                data.order_number = order_number
                request.session['order_number'] = order_number
...
# This function will handle cash on delivery payment
def cod(request):
    current_user = request.user
    # generate order number
    order_number = request.session["order_number"]
...
# This function will take to order_complete page
